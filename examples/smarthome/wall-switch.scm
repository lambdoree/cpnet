...
(define-execution run-smarthome-scenario
  ...
  (trigger WallSwitchSystemDef.wall-switch.button_press #t)
  (run)
  ...
  (trigger WallSwitchSystemDef.thermostat.current_temp 75)
  (run)
  (show-state "Temp rises, AC turns on, total power increases")
  (trigger WallSwitchSystemDef.thermostat.current_temp 68)
  (run)
  (show-state "Temp drops, AC turns off")

  (show-state "--- Thermostat Scenario (Heating) ---")
  (trigger WallSwitchSystemDef.thermostat.mode 'heat)
  (run)
  (show-state "Mode set to heat, nothing happens yet")
  (trigger WallSwitchSystemDef.thermostat.current_temp 65)
  (run)
  (show-state "Temp drops, Heater turns on")
  (trigger WallSwitchSystemDef.thermostat.current_temp 71)
  (run)
  (show-state "Temp rises, Heater turns off")

  (show-state "--- Security System Scenario ---")
  (trigger WallSwitchSystemDef.security-system.status 'armed_away)
  (run)
  (show-state "Security system is armed")
  (trigger WallSwitchSystemDef.door-sensor.state 'open)
  (run)
  (show-state "Door opens, alarm sounds")
  (trigger WallSwitchSystemDef.door-sensor.state 'closed)
  (run)
  (show-state "Door closes, alarm remains on")
  (trigger WallSwitchSystemDef.security-system.status 'disarmed)
  (run)
