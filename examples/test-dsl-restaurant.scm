...
(define-cpnet-system RestaurantSys
  ...
  (category recipe-book
		...
		(let ((dish (car new))
		      (ings (cadr new))
		      (old  (cell-value (hash-ref recipe-book-cells 'internal-recipes))))
		  (cons (acons dish ings old)
			(list (make-effect 'set-value (cons src #f)))))))
	     ((prop p-get-ingredients in-dish-name -> out-ingredients)
	      (lambda (dish src)
		(let* ((recipes (cell-value (hash-ref recipe-book-cells 'internal-recipes)))
  ...
  (category inventory
		...
		(if (not decr)
		    (cons #f '())
		    (let* ((inv0   (cell-value (hash-ref inventory-cells 'internal-inventory)))
  ...
  (category forecasting
	    ...
	    (propagators
	     ((prop p-update-forecast in-forecast-dish -> out-ingredient-forecast)
	      (lambda (dish src)
		(let* ((counts0 (cell-value (hash-ref forecasting-cells 'internal-sales-counts)))
                       (old-pair (assoc dish counts0))
                       (old (if old-pair (cdr old-pair) 0))
                       (counts1 (cons (cons dish (1+ old))
                                      (filter (lambda (p) (not (equal? (car p) dish))) counts0)))
                       (recipe-pair (assoc dish (cell-value (hash-ref recipe-book-cells 'internal-recipes))))
                       (recipe (and recipe-pair (cdr recipe-pair)))
                       (forecast
                        (and recipe
                             (map (lambda (pr)
                                    (cons (car pr) (* (cdr pr) (1+ old))))
                                  recipe))))
		  (cons (if (null? forecast) #f forecast)
			(list (make-effect 'set-value
					   (cons (hash-ref forecasting-cells
							   'internal-sales-counts)
						 counts1))
			      (make-effect 'set-value (cons src #f)))))))))

  (connections
   (fan-out     p-fan-out-sales sales.public-dish-sold
                (recipe-book.in-dish-name forecasting.in-forecast-dish))
   (connector   p-recipes->inventory recipe-book.out-ingredients
                inventory.public-ingredients-decrement)
   (propagator  p-inventory->alert inventory.out-low-stock-trigger
                inventory.out-low-stock-trigger
                (lambda (alerts src)
                  (if alerts
                      (cons #f
                            (cons (make-effect 'set-value (cons src #f))
                                  (map (lambda (e)
                                         (make-effect 'display
                                                      (format #f
                                       ...
                                       alerts)))
                      (cons #f '()))))
   (propagator  p-display-forecast forecasting.out-ingredient-forecast
                forecasting.out-ingredient-forecast
                (lambda (fc src)
                  (if fc
                      (cons #f
                            (list (make-effect 'set-value (cons src #f))
  ...
  (execution
   (trigger recipe-book.in-new-recipe
            (list "Pizza" `(("Dough" . 3) ("Tomato" . 2) ("Cheese" . 2))))
