...
(define (rule-AA! net a-name b-name pass-through?)
  (when (and (eq? (node-agent net a-name) 'A)
    ...
    (let* ((a_p (cons a-name 'p)) (a_l (cons a-name 'l)) (a_r (cons a-name 'r))
      ...
      (if (or pass-through? (and (not X) (not F) (not Y)))
          (begin
            (debugf 2 "rule-AA!: skipping on ~a and ~a (pass-through: ~a, no-aux: ~a)\n" a-name b-name pass-through? (and (not X) (not F) (not Y)))
            #f)
...
(define (get-literal-value node-name)
  ...
  (let ((s (symbol->string node-name)))
    (cond
     ...
     ((string-prefix? "trig-num-" s)
      (let* ((parts (string-split (substring s 9) #\-))
             (val-str (string-join (reverse (cdr (reverse parts))) "-")))
        (string->number val-str)))
     ;; string families
     ((string-prefix? "str-" s) (string-join (string-split (substring s 4) #\_) " "))
     ((string-prefix? "trig-str-" s)
      (let* ((parts (string-split (substring s 9) #\-))
             (val-str (string-join (reverse (cdr (reverse parts))) "-")))
        (string-join (string-split val-str #\_) " ")))
     ;; Fallback
       ...
       (let ((s (symbol->string n)))
         ...
         (when (or (string-contains s "eq") (string-contains s "lt") (string-contains s "gt"))
           (let* ((l-ep (peer net (cons n 'l)))
                  ...
                  (l-val (and l-ep (resolve-literal-ep net l-ep 8)))
                  (r-val (and r-ep (resolve-literal-ep net r-ep 8))))
...
;; A-A 병합을 위한 재작성 패스입니다 (현재는 스텁).
(define (rewrite-pass-AA-merge! net)
  ;; Stub for optional A-A merge
  #f)
